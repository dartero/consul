class Admin::BudgetInvestmentMilestonesController < Admin::BaseController
  ...
  before_action :load_budget_statuses, only: [:index, :new, :create, :edit, :update]
  ...
  def load_budget_statuses
    @statuses = Budget::Investment::Status.where(budget_id: @investment.budget.id)
  end
